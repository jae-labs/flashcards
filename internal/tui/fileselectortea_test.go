...
import (
	...
	"github.com/jae-labs/flashcards/internal/tui/keys"
)
...
func TestFileSelectorModel_Update_ToggleAllFiles(t *testing.T) {
	...
	if !m.selected[fileOne] || !m.selected[fileTwo] {
	...
	if m2.selected[fileOne] || m2.selected[fileTwo] {
...
func TestFileSelectorModel_Update_Quit(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := NewFileSelectorModel([]string{fileOne})
...
func TestFileSelectorModel_View_AfterConfirm(t *testing.T) {
	model := NewFileSelectorModel([]string{fileOne})
...
func TestFileSelectorModel_GetSelectedFiles(t *testing.T) {
	...
	model := NewFileSelectorModel(files)
	model.selectedFiles = []string{fileOne}
	...
	if result[0] != fileOne {
		t.Errorf("GetSelectedFiles()[0] = %q, want %q", result[0], fileOne)
	}
}
